...
class Server(BaseServer):
    ...
    def _nova_aggregation(self, dg_weights, local_weights, client_sizes):
        ...
        for k in ag_weights.keys():
            for idx in local_weights.keys():
                ag_weights[k] = (ag_weights[k].to('cpu')+
                    local_weights[idx][k].to('cpu') * c_1 * p_i[idx] / self.local_a[idx]
                )
    ...
    def _init_nova_dependencies(self):
        ...
        root = os.path.join("./data", self.data_distributed["data_name"])
          

        for i in range(self.n_clients):
            tau_i[i] = self.local_epochs * len(
                list(DATA_LOADERS[self.data_distributed["data_name"]](root=root, train=True, batch_size=50, dataidxs=self.data_distributed["local"][i]["train_idxs"]))
            )
