...
def set_path(args):
    ...
    for tmp_path in ['./results/logs', './results/model_checkpoints']:
        path = os.path.join(tmp_path, args.dataset, args.model)
        if not os.path.isdir(path):
            os.makedirs(path)
    
    # set file path
    args.log_path = './logs/%s/%s/non-iid_%s/[%s]seed%s_clients%s-%s_rounds%s_epochs%s' % (args.dataset, args.model, args.non_iid, args.algorithm, args.seed, args.num_clients, args.clients_per_round, args.num_rounds, args.num_epochs)
    if args.exp_name:
        args.log_path += '_%s' % args.exp_name
    args.log_path += '.csv'
    
    args.checkpoint_path = './model_checkpoints/%s/%s/non-iid_%s/[%s]seed%s_clients%s-%s_rounds%s_epochs%s' % (args.dataset, args.model, args.non_iid, args.algorithm, args.seed, args.num_clients, args.clients_per_round, args.num_rounds, args.num_epochs)
    if args.exp_name:
