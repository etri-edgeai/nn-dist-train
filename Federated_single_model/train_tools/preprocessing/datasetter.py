...
#         net_dataidx_map = centralized_partition(all_targets)
    ...
    if partition.method == "iid":
        ...
        net_dataidx_map = sharding_partition(all_targets, n_clients, partition.shard_ratio)
...
#         net_dataidx_map = sharding_max_partition(all_targets, n_clients, partition.K)
    ...
    traindata_cls_counts = record_net_data_stats(net_dataidx_map, all_targets)
    
#     data_map = net_dataidx_map_counter(net_dataidx_map, all_targets)
#     logging.info("traindata_cls_counts = " + str(data_map))
    ...
    for client_idx, dataidxs in net_dataidx_map.items():
        ...
        logging.info("client_idx = %d, local_sample_number = %d" % (client_idx, local_loaders[client_idx]["datasize"]))
        logging.info("client_idx = %d, batch_num_train_local = %d" % (
            client_idx, len(local_loaders[client_idx]["train"])))
    ...
    logging.info("train_dl_global number = " + str(len(global_loaders["train"])))
    logging.info("test_dl_global number = " + str(len(global_loaders["test"])))
...
def sharding_partition(all_targets, n_clients, shard_per_ratio):
    
    num_shards= int(len(np.unique(all_targets))*shard_per_ratio)*n_clients 
    num_imgs= int(int(len(all_targets))/num_shards)
    
    idx_shard = [i for i in range(num_shards)] 
    net_dataidx_map = {i: np.array([]) for i in range(n_clients)} 
    ...
    for i in range(n_clients):
        rand_set = set(np.random.choice(idx_shard, int(len(np.unique(all_targets))*shard_per_ratio), replace=False))
