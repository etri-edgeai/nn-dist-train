...
class Cutout(object): #_data_transforms_cifar100()에 적용됨!!
...
def load_cifar100_data(datadir): #partition_data에 쓰임!!
...
def partition_data(dataset, datadir, partition, n_nets, alpha, ratio_shard):
    ...
    if partition == "homo":
        ...
        batch_idxs = np.array_split(idxs, n_nets)#n_nets는 total client number에 대응될 것(cluster 개수)!!
    ...
    elif partition == "shard": #각 client는 최대 class_num*ratio_shard개의 label을 가질 수 있는 상황(Cifar 100기준 20퍼센트 비율!!)
        num_shards= int(class_num*ratio_shard)*n_nets #ratio_shard=0.2, n_nets=100으로 짜인 상황!!->2000
        num_imgs=int(n_train/num_shards) #25
        
        idx_shard = [i for i in range(num_shards)] #length=2000
...
#         net_dataidx_map = {}        
        ...
        idxs_labels = idxs_labels[:, idxs_labels[1, :].argsort()]#label이 0부터 99까지 monotone increasing하게 idx 재배정
        idxs = idxs_labels[0, :]#label이 0부터 99까지 monotone increasing하게 idx 재배정

        # divide and assign 20 shards/client(client 100명이라서)
        for i in range(n_nets):
            rand_set = set(np.random.choice(idx_shard, int(class_num*ratio_shard), replace=False))#0~1999번쨰 idx shard 중 없앨 class_num*ratio_shard=20개의 shard idx 추린다.#num shard=2000, num of clinet: 100이라서 정확히 모든 데이터가 쪼개어져 배정.
            idx_shard = list(set(idx_shard) - rand_set)#20개 없앤 것 반영.각 shard는 동일한 label을 갖게 된다!!
            for rand in rand_set:#i번째 client에 class_num*ratio_shard=20개의 shard를 배정해 500개의 image 준다!!=> non-iid
        ...
        min_size = 0
        K = class_num  #number of class, 100
...
def get_dataloader_CIFAR100(datadir, train_bs, test_bs, dataidxs=None):#train data에만 dataidxs 적용!!
    ...
    transform_train, transform_test = _data_transforms_cifar100()#transformation information!!
...
def load_partition_data_cifar100(dataset, data_dir, partition_method, partition_alpha, client_number, batch_size, ratio_shard):
    X_train, y_train, X_test, y_test, net_dataidx_map, traindata_cls_counts = partition_data(dataset,
                                                                                             data_dir,
                                                                                             partition_method,
                                                                                             client_number,
    ...
    for client_idx in range(client_number):#각 client_idx마다 실행!!
        ...
        # training batch size = 64; algorithms batch size = 32
        #dataloader의 형태이다. not data set!!
        train_data_local, test_data_local = get_dataloader(dataset, data_dir, batch_size, batch_size,
                                                 dataidxs) #dataidxs를 적용하여  train local data를 적용, test data는 global하다!!
